    ...
    public class ArgsOptionsBuilder
    {
        ...
        private void ProcessArguments(string[] args)
        {
            _optionSet = new OptionSet
            {
                ...
                {
                    "i|ignore=",
                    "ignore directories whose name matches the given {REGEX_PATTERN} (repeat for multiple ignores)",
                ...
                {
                    "H|fixhintpaths",
                    "convert hint paths in csproj files to point to nuget packages folder relative to generated sln",
                    v => _options.FixHintPaths = true
                },
                {
                    "U|restorehintpaths",
                    "unconvert hint paths in csproj files to point to their original nuget packages folder",
        ...
        private VisualStudioVersion TryParseVersionNumber(string versionNumber)
        {
            ...
            if (parsedVersion == null) parsedVersion = VisualStudioVersion.VS2017;
