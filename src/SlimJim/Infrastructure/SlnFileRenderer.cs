...
namespace SlimJim.Infrastructure
{
    public class SlnFileRenderer
    {
        ...
        {
            var stream =
                typeof(SlnFileRenderer).Assembly.GetManifestResourceStream("SlimJim.Templates.SolutionTemplate.st");
            TemplateGroup = new StringTemplateGroup("SlnTemplates");
            using (stream)
            {
                TemplateGroup.DefineTemplate("SolutionTemplate",
                    new StreamReader(stream ?? throw new InvalidOperationException()).ReadToEnd());
            }
        }
